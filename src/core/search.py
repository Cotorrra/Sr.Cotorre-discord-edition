...
def card_search(query, cards, keyword_func, allow_empty=False):
    ...
    if "cycle" in query and query["cycle"]:
...
def card_filter(query: dict, cards: List[dict]):
    """
...
def find_by_id(code: str, cards: List[dict]):
    """
...
def filter_by_subtext(card: dict, subname: str):
    """
...
def hits_in_string(query: str, find: str):
    ...
    for w1 in set1:
        for w2 in set2:
            w1_c = sub(r"[^a-z\d]", "", unidecode(w1))
            w2_c = sub(r"[^a-z\d]", "", unidecode(w2))
